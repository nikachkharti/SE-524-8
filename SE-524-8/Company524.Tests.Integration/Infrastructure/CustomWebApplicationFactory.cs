...
namespace Company524.IntegrationTests.Infrastructure;

/// <summary>
/// ეს კლასი ქმნის ჩვენი API-ს "ტესტ ვერსიას".
///
/// WHAT IS THIS?
/// WebApplicationFactory = ASP.NET Core-ის ჩაშენებული კლასი,
/// რომელიც ჩვენი სრული აპლიკაცია მეხსიერებაში (RAM-ში) ამუშავებს.
///
/// WHAT DO WE REPLACE?
/// 1. SQL Server     → EF Core InMemory Database
/// 2. Config files   → In-memory key/value config (JWT, ConnectionString, etc.)
/// 3. IEmailService  → Mock (email-ს ნამდვილად არ გზავნის)
/// 4. ISmtpClient    → Mock (SMTP კავშირი არ სჭირდება)
///
/// ROOT CAUSE OF THE LATEST ERROR:
/// appsettings.Development.json contains JS-style comments (//) which are
/// invalid JSON — the file fails to parse and JWT:Secret comes back null.
/// Encoding.GetBytes(null) then throws ArgumentNullException.
/// Fix: inject all required config values in-memory so no file is needed.
/// </summary>
public class CustomWebApplicationFactory : WebApplicationFactory<Program>
{
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        // Tell the app it's in "Testing" environment so Program.cs
        // skips the SQL Server migration block.
        ...
        {
            // Clear every config source that was already added
            // (appsettings.json, appsettings.Development.json, env vars, etc.)
            config.Sources.Clear();

            // Add a clean, comment-free in-memory config
        ...
        builder.ConfigureServices(services =>
        {
            ...
            // -------------------------------------------------------
            var smtpDescriptor = services.SingleOrDefault(
                d => d.ServiceType == typeof(ISmtpClient));
